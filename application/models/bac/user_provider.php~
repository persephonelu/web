...
class User_provider extends CI_Model {
    ...
    //如果不一致，跳转到错误页面,后续设计检测token来验证
    //注： admin账号58100533@qq.com不需要检测登录
    public function check_login_restful($email, $token="")
    {
        //admin账号，不需要登录
        ...
        if ( $session_email ) //如果session设置了email
        {
        ...
        //其他情况全都跳转到错误页面
        $error_url = base_url() . "error?ec=-2&ei=no_log_in";
    ...
    {
        $sql = "select email,nickname from member where email='$email'";
        $result = $this->db->query($sql)->result_array();
        return $result[0];
    }
